...
- Memory default is 3 GiB (Postgres + the Windmill server + worker processes). **Raising the memory
...
- **Bundling a DB makes you the OOM and `pg_upgrade` owner**: Postgres shares the app's one memory
  limit with the server and every worker process (set `shared_buffers`/`work_mem` low; document a
  floor; size the worker count from the limit so PG keeps headroom), and a PG major bump needs an
...
- **Don't ship `MODE=standalone`** — it's a dev-only single process (and pins `NUM_WORKERS` to 1; the
  Windmill maintainer flagged it as not-for-production). Run the supported split even in one container:
  one `MODE=server` process + N `MODE=worker` processes under supervisor, generating the worker units
  per-boot so the count can scale with the memory limit. `MODE`/`PORT` go per-unit, never global, or
  workers fight the server for port 8001. (Revised in v1.1.0 — see ADR 0001 / PACKAGING-NOTES.)
